...
fn generate_x11_constants(output_dir: &str) -> Result<(), GenerateErr> {
    ...
    let inputs = [("extensions/XI2.h", "xinput2.constants.rs")];
...
fn main () {
    // constants auto-generated from X11 headers are checked
    // into the source tree to enable building the package on systems
    // without X11. They can be regenerated by setting the REGENERATE_BINDINGS
    // env var prior to running 'cargo build'
    if env::var("REGENERATE_BINDINGS").is_ok() {
        match generate_x11_constants("../src/generated") {
            Ok(_) => (),
    ...
    match env::var("CARGO_MANIFEST_DIR") {
        Ok(ref dir) if dir.ends_with("x11") => {
